...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`[creative-replicator-v2] Submitting job to VPS at ${vpsUrl} for ${videoId}`);

      // 1. Submit Job
      const submitRes = await fetch(`${vpsUrl}/execute`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: config.projectId || 'replicator_v2',
          sources: inputUrls,
          transitions: ['fade', 'whip-pan', 'zoom'], // Default variety
          clipDuration: config.pacing === 'fast' ? 1.5 : (config.pacing === 'slow' ? 5 : 3),
          maxDuration: 30,
          outputName: videoId
        })
      });
      ...
      if (!submitRes.ok) {
        ...
        throw new Error(`VPS Submission Failed (${submitRes.status}): ${errText}`);
      }

      const submitData = await submitRes.json();
      const jobId = submitData.jobId;
      console.log(`[creative-replicator-v2] VPS Job Accepted: ${jobId}`);
      ...
      while (attempts < maxAttempts) {
        ...
        const statusRes = await fetch(`${vpsUrl}/jobs/${jobId}`);
        if (!statusRes.ok) continue;

        const jobState = await statusRes.json();

        if (jobState.status === 'done') {
          generatedUrl = jobState.outputUrl;
          // Fix URL if relative
          if (generatedUrl && generatedUrl.startsWith('/')) {
            generatedUrl = `${vpsUrl.replace('/api', '')}${generatedUrl}`;
          }
          break;
        } else if (jobState.status === 'error') {
          throw new Error(`VPS Rendering Failed: ${jobState.error?.message || 'Unknown error'}`);
        }
      }
      ...
      const validation = await validateUrl(generatedUrl);
      if (!validation.valid) {
        console.warn(`[Video Validation] URL reachable but check warning: ${validation.reason}`);
        // Don't fail hard if it's just a size check on valid URL, but proceed with caution
      }
      pipelineStatus.url_validation = 'success';

      return {
        url: generatedUrl,
        thumbnail: generatedUrl.replace('.mp4', '_thumb.jpg'),
        duration: 15, // TODO: Get actual duration from VPS jobState
      };
